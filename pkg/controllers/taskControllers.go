...
func CreateTask(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		createdTask, err := task.CreateTask(userID, db)
		if err != nil {
...
func GetTasksByUserId(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var task models.Task
		tasks, err := task.GetTaskByUserId(userID, db)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
...
func GetTaskById(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var task models.Task
		taskDetails, err := task.GetTaskById(taskID, db)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
...
func UpdateTask(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		updatedTask, err := task.UpdateTask(taskID, db)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
...
func ExportTasksExcel(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		userIDVal, exists := c.Get("user_id")
		if !exists {
			c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
